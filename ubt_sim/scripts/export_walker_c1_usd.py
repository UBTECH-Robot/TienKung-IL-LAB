#!/usr/bin/env python3
"""Import Walker C1/Astron URDF in Isaac Sim and export it as USD.

Run inside the Isaac Sim container:

    /isaac-sim/python.sh -u /ubt_sim/scripts/export_walker_c1_usd.py --headless
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Export Walker C1 URDF to USD.")
    parser.add_argument(
        "--urdf",
        type=Path,
        default=Path("/ubt_sim/assets/robots/walker_c1/walker_astron_v2_hand_v3_no_sixforce_mesh.urdf"),
        help="Path to the Walker C1 URDF inside the Isaac container.",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("/ubt_sim/assets/robots/walker_c1/walker_c1.usd"),
        help="Output USD path inside the Isaac container.",
    )
...
def main() -> None:
    ...
    if not urdf_path.exists():
        raise FileNotFoundError(f"Walker C1 URDF not found: {urdf_path}")
    ...
    print(f"[OK] Exported USD: {output_path}")
