...
【使用示例】
    ...
    # 查询指定身体关节当前状态
    python3 walker_s2_joint_test.py --joints R_elbow_yaw_joint L_shoulder_pitch_joint

    # 移动指定身体关节到目标角度
    python3 walker_s2_joint_test.py --move R_elbow_yaw_joint=0.5 --duration 2.0

    # 多关节同时移动
    python3 walker_s2_joint_test.py --move R_elbow_yaw_joint=0.5 L_shoulder_pitch_joint=-0.3

    # 相对当前位置偏移
    python3 walker_s2_joint_test.py --shift R_elbow_yaw_joint=+0.1

    # 持续监控指定关节（10Hz 刷新）
    python3 walker_s2_joint_test.py --monitor --joints head_pitch_joint head_yaw_joint

    # 交互模式（Python REPL）
    python3 walker_s2_joint_test.py --interactive --joints R_elbow_yaw_joint

    # ---- 手指关节 ----

    # 查询左手手指状态
    python3 walker_s2_joint_test.py --hand left --print

    # 移动左拇指
    python3 walker_s2_joint_test.py --hand left --hand-move thumb_swing=0.5

    # 右食指偏移
    python3 walker_s2_joint_test.py --hand right --hand-shift index_mcp=+0.2

    # 设置整手姿态（7个角度值，按 V4_HAND_*_JOINTS 顺序）
    python3 walker_s2_joint_test.py --hand left --hand-pose 0.5 0.3 0.1 0.8 0.8 0.8 0.8

    # 预设姿态：张开 / 握拳
    python3 walker_s2_joint_test.py --hand both --hand-open
    python3 walker_s2_joint_test.py --hand left --hand-close

    # 双手周期波形运动
    python3 walker_s2_joint_test.py --hand both --hand-wave

    # 监控手指关节
    python3 walker_s2_joint_test.py --monitor --joints left_thumb_swing left_index_mcp
"""
...
try:
    from .walker_s2_controller import (
    BODY_JOINT_LIMITS,
    BODY_JOINT_NAMES,
    DEFAULT_LOCK_JOINTS,
    V4_HAND_CLOSE_POSE,
...
使用示例：
  ...
  # 移动指定身体关节到目标角度
  python3 walker_s2_joint_test.py --move R_elbow_yaw_joint=0.5
...
可用手指关节名（右手）：
  """ + "\n  ".join(V4_HAND_RIGHT_JOINTS),
    ...
    # 无操作模式时默认打印
    if not has_body_action and not has_hand_action:
        cli_args.print = True
