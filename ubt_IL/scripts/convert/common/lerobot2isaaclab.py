...
def denormalize_lerobot_to_isaaclab_radians(joint_values_lerobot: np.ndarray) -> np.ndarray:
    ...
    Supported dimensions:
      - 6:  single arm (6 joints)
      - 12: bimanual (6 joints per arm)
      - 14: dual arm only (7 joints per arm, no hands)
      - 26: dual arm + hands (7+6 per side, hands pass through untransformed)
    Other dimensions raise ValueError.
    """
    ...
    if dimension == 6:
        # single arm: denormalize all 6 joints
        ...
        # bimanual: denormalize 6 joints per arm
        ...
        # dual arm only (7 joints per arm): denormalize first 6 of each 7-joint arm
        ...
        # joint 6 (index 6 and 13) passes through without denormalization

    elif dimension == 26:
        # dual arm + hands: denormalize arm joints (first 7 of each 13-joint side)
        # Layout: [left_arm(7), left_hand(6), right_arm(7), right_hand(6)]
        for arm_offset in (0, 13):
            for joint_index in range(6):
                isa_min, isa_max = ISAACLAB_LIMITS_DEG[joint_index]
                le_min, le_max = LEROBOT_LIMITS_DEG[joint_index]
                column_index = arm_offset + joint_index
        ...
        # joint 6 (7th arm joint) and all hand joints pass through without denormalization

    else:
        raise ValueError(
            f"Unsupported action dimension {dimension}. "
            f"Supported: 6 (single arm), 12 (bimanual), 14 (dual arm), 26 (dual arm + hands)."
        )
