        ...
        with h5py.File(episode_path, "r") as file:
            ...
            for lerobot_key, field_spec in mapping.items():
                if isinstance(field_spec, list):
                    ...
                    if encoding == "jpeg":
                        # cv2 has no IMREAD_COLOR_RGB; decode BGR then convert to RGB
                        # (LeRobot stores video frames as RGB). np.frombuffer matches
                        # the depth branch and handles raw JPEG bytes from the object array.
                        images = [
                            cv2.resize(
                                cv2.cvtColor(
