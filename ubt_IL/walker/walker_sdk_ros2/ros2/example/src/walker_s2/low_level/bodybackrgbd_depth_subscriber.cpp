...
class BodyBackRgbdDepthSubscriber : public rclcpp::Node
{
 public:
  BodyBackRgbdDepthSubscriber() : Node("body_back_depth_subscriber")
  {
    subscription_ = this->create_subscription<shm_msgs::msg::Image1m>(
        "/sensor/camera/body_back_rgbd/depth/raw",  // 话题名称
        rclcpp::QoS(rclcpp::KeepLast(10)).best_effort(),
        std::bind(&BodyBackRgbdDepthSubscriber::topic_callback, this,
                  std::placeholders::_1));
  }
...
int main(int argc, char *argv[])
{
  ...
  rclcpp::spin(std::make_shared<BodyBackRgbdDepthSubscriber>());
  rclcpp::shutdown();
  return 0;
}
