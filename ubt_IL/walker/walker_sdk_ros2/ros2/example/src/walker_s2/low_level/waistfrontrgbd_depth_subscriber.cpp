...
class WaistFrontRgbdDepthSubscriber : public rclcpp::Node
{
 public:
  WaistFrontRgbdDepthSubscriber() : Node("waist_front_depth_subscriber")
  {
    subscription_ = this->create_subscription<shm_msgs::msg::Image1m>(
        "/sensor/camera/waist_front_rgbd/depth/raw",  // 话题名称
        rclcpp::QoS(rclcpp::KeepLast(10)).best_effort(),
        std::bind(&WaistFrontRgbdDepthSubscriber::topic_callback, this,
                  std::placeholders::_1));
  }
...
int main(int argc, char *argv[])
{
  ...
  rclcpp::spin(std::make_shared<WaistFrontRgbdDepthSubscriber>());
  rclcpp::shutdown();
  return 0;
}
