...
class FishEyeRightSubscriber : public rclcpp::Node
{
 public:
  FishEyeRightSubscriber() : Node("fish_eye_right_subscriber")
  {
    subscription_ = this->create_subscription<shm_msgs::msg::Image2m>(
        "/sensor/camera/fisheye_right/image/raw",  // 话题名称
        rclcpp::QoS(rclcpp::KeepLast(10)).best_effort(),
        std::bind(&FishEyeRightSubscriber::topic_callback, this,
                  std::placeholders::_1));
  }
...
int main(int argc, char *argv[])
{
  ...
  rclcpp::spin(std::make_shared<FishEyeRightSubscriber>());
  rclcpp::shutdown();
  return 0;
}
