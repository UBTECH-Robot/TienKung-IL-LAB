...
在线控制器另有 `--motion-speed`，默认 `1.5`，只压缩机械臂插值时间；闭手、苹果落稳等待和
静态防滑检查不缩短。保守速度可用 `--motion-speed 1.0`，进一步实验可用 `2.0`。记录的 HDF5
会在根属性保存本局 `motion_speed`。GUI 下 `100 Hz + 1.5x` 完整回归耗时约 60 秒，原 30 Hz
版本约 120 秒；苹果 `HELD/STABLE`，最终距盘心 `0.036 m`，结果 `1/1 SUCCESS`。
