...
**⚠️ 2026-07-20 用户重要反馈——当前实现方式不合格，必须重做：**
...
- `teleoperation/control/walker_c1/pick_place_controller.py`：之前写的 IK 编排版抓放,标记"experimental",里面的路点/姿态参数是拍脑袋/从录像反推的，**不要直接照抄这些参数**，但里面的框架结构（分阶段 approach/descend/close/lift/carry/place）可以参考；
- bridge 已经支持 `/sim/cmd_set_object_pose`（仿真专用，命令苹果瞬移到指定世界坐标）和 `/sim/object_state`（回读苹果位置、机器人根位姿、右手各指链节世界坐标）。

**下一步具体建议**：重写 `pick_place_controller.py`（或新文件），去掉所有"抄录像角度"的逻辑，改成：go_ready → 读 `/sim/object_state` 拿苹果真实世界坐标 → 转到机器人基座坐标系 → IK 解算抓取路点（可以参考 S2 的球面采样思路，或者先用一个固定的相对偏移+掌心朝下姿态起步，成功后再加采样） → 闭环对准（`mouth_center_w()` 已实现，罩口对准苹果这个机制之前验证有效）→ 五指合围（`[0.75,0.85/0.9,...]` 这组参数之前验证有效可以先沿用）→ 提起校验（照抄 S2 的双阈值判定）→ 没抓住就重新读苹果位置原地重试 → 抓住了再规划去盘子的路径 → 放下 → 归位。
...
- **场景**：桌子/盘子用隐形碰撞体对齐视觉家具，装饰苹果已用 `scene_v2_c1.usda` 覆盖层隐藏，刚体苹果 r=0.022（实测手部罩口有效孔径 5-6cm，7cm 的球装不下，这是之前吃过的教训）。
- **物理配置**：`config.py` 里手部增益已回退到 in-process 时代验证过的 10/1/2（力矩 2N·m 上限），force-drive USD（`walker_c1_force_drive.usd`）已经是标准配置；`walker_c1_force_drive_grip.usd`（高摩擦版本）是调试产物，已验证摩擦不是问题根源，**默认不要用它**。
